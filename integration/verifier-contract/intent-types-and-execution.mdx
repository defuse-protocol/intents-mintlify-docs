...
Intents are submitted as JSON objects in a payload. Here's an example [Transfer](https://near.github.io/intents/defuse_core/intents/tokens/struct.Transfer.html) intent for USDC tokens from Alice to Bob:
...
To create a valid, signed intent for the `execute_intents` function, wrap the payload in a message string. Note that the message is the same JSON serialized as a one-liner with escaped quotes:
...
The `recipient` field prevents replay attacks on other copies of the Verifier. See [Signing Intents](/integration/verifier-contract/signing-intents) for cryptographic details.
...
<Info>
Token IDs are specified in [Multi Token format](/integration/verifier-contract/deposits-and-withdrawals/balances).
</Info>
...
The user declares willingness to have a set of changes applied to their tokens. For example, a trade of 100 token A for 200 token B can be represented as `{"A": -100, "B": 200}`.
